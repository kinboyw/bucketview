<template>
  ...
  >
            ...
            <div class="transfer-row">
              <div class="transfer-info">
                ...
                <span class="transfer-filename"
                  :class="{
                    'transfer-error': value.status == 'error' || value.status == 'cancel',
                  }"
                >{{ value.name }}</span>
                <a-tooltip>
                  <template #title>
    ...
    <div class="transfer-fab-wrapper" @mouseenter="transferFabHover = true" @mouseleave="transferFabHover = false">
      <div class="transfer-fab" :class="{ 'transfer-fab-visible': transferFabHover || transferLength > 0 }" @click="handleShowTransferDrawer()" title="传输列表">
...
export default defineComponent({
  ...
  setup() {
    ...
    // 传输列表
    const transferDrawerState = reactive({
      visible: false,
    });
    ...
    const handleRecoverInterrupted = () => {
      ...
      for (const job of interrupted) {
        ...
        if (job.type === 'upload') {
          ...
          storage.getObject(defaultStorage, t);
        }
        recovered++;
      }
      if (recovered > 0) {
        notification['success']({ message: `已恢复 ${recovered} 个传输任务` });
      } else {
        notification['info']({ message: '没有需要恢复的传输任务' });
      }
    };
    ...
    const handleTransferRetry = (uid: string) => {
      ...
      // For downloads: try resume if partial file exists
      if (t.type === 'download' && t.localPath && t.totalBytes) {
        ...
        if (localSize !== null && localSize > 0 && localSize < t.totalBytes) {
          // Partial file exists and is smaller than total → resume
          Object.assign(t, {
            status: 'waiting',
          ...
          const resumeOptions = Object.assign({}, toRaw(ensureTransferRecordContext(t)), {
            resumeFrom: { filePath: t.localPath, downloaded: localSize, total: t.totalBytes },
          });
          storage.getObject(defaultStorage, resumeOptions);
          return;
        }
        // No partial file or file is complete/corrupt → delete and start fresh
        if (localSize !== null) {
          // File exists but is same size or larger → force overwrite
          Object.assign(t, {
            status: 'waiting',
          ...
          const overwriteOptions = Object.assign({}, toRaw(ensureTransferRecordContext(t)), {
            forceOverwrite: true,
          });
      ...
      // No localPath or upload → standard retry
      Object.assign(t, {
        status: 'waiting',
        percentage: undefined,
        speed: undefined,
      ...
      if (t.type === 'upload') {
        storage.putObject(defaultStorage, toRaw(ensureTransferRecordContext(t)) as TransferInfo);
      } else {
    ...
    const handleShowTransferDrawer = (visible?: boolean) => {
      transferDrawerState.visible = visible ?? !transferDrawerState.visible;
    };
