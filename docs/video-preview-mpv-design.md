# 广播影视级专业视频预览（ProRes / MOV）支持方案

## 1. 现状与痛点

- **文件特征**：影视制作与广电常用的 `Apple ProRes 422 (HQ/LT/Proxy/4444)`，通常封装为 `.mov` 格式，伴随多轨未压缩 PCM 音频（`pcm_s16le`）及时间码流（`tmcd / 0x64636D74`），码率普遍在 100Mbps ~ 250Mbps 以上。
- **核心痛点**：
  1. **解码器缺失**：Chromium / Web 规范仅支持网络常用流媒体格式（H.264/H.265/VP9/AV1 + AAC/Opus），对 ProRes 与 PCM 音频在 `<video>` 中无法硬解或软解；
  2. **时间码轨兼容问题**：FFmpeg / 解复用器在默认探测时若未显式忽略 `tmcd` 流，会报错 `Unsupported codec with id 0 for input stream 7`；
  3. **转码负载高**：150Mbps+ 的 ProRes 实时软件转码极其消耗 CPU，拖拽 Seek 体验差。

---

## 2. 演进路线规划

### 阶段一：短期方案（方案 3 - 独立嵌入式 mpv 二进制进程播放器）
- **定位**：快速落地、免编译 C++ 原生扩展、稳定性最高。
- **架构设计**：
  1. 随包分发或按需静默拉取轻量裁剪版 `mpv.exe`（~30MB，类似已实现的 `rclone` 机制）；
  2. 当用户预览 `.mov`（或检测到 ProRes/DNxHD 编码）时，通过主进程拉起 `mpv` 进程；
  3. 通过 `mpv --input-ipc-server=\\.\pipe\mpv-pipe-bucketview` 实现 JSON-IPC 控制（播放/暂停/Seek/音量/时间码获取）；
  4. 启动参数配置：
     - `--hwdec=auto`：硬件加速直解（NVDEC/D3D11VA/VideoToolbox），轻松拉动百兆码率；
     - `--demuxer-lavf-o=discard=data`：自动丢弃/忽略 tmcd 时间码数据流，彻底避免解码器报错；
     - 直接输入 S3 Presigned URL 或通过 FUSE 本地挂载路径加载。

### 阶段二：长期方案（方案 1 - libmpv + Node Native Addon 内嵌绘制）
- **定位**：极致无缝的 UI 融合体验。
- **架构设计**：
  1. 集成 `libmpv` 动态库（`.dll` / `.dylib`）；
  2. 通过 C++ Node Addon 或 WebGL / WebGPU 共享纹理离屏渲染，直接渲染至 Vue 组件的 `<canvas>` / 自定义 WebGL 容器中；
  3. 复用 Vue 编写的统一深浅色控制栏、选时跳帧、色彩空间调节（Rec.709 / BT.2020 / HDR）。

---

## 3. 插件中心与一键安装架构（已落地）

1. **统一系统插件接口 (`PluginManager`)**：
   - 将 `rclone`（文件系统驱动）、`WinFsp`（Windows 用户态文件系统内核驱动）与 `mpv`（音视频解码播放器）纳入统一的插件体系管理；
   - 默认展示系统受支持的插件卡片，清晰展示安装状态、运行来源（系统预装 / 本地自动管理 / 自定义路径）与版本信息。
2. **全自动免包管理器安装流水线**：
   - **MPV 免包管理器直链安装**：移除对 `winget` 的依赖，直接下载官方便携发布 zip 包解压至应用数据目录（如 `%APPDATA%\BucketView\bin\mpv\`）；
   - **WinFsp 一键静默安装**：Windows 平台支持一键下载官方发布 MSI 安装包并静默安装，彻底打通 rclone Windows 本地磁盘挂载全链路；
   - **国内 GitHub 加速代理降级**：所有 GitHub Release 资产（mpv / WinFsp / 应用更新包）优先尝试官方直连，连通失败时自动降级走加速镜像（`https://gh.kinboy.wang/` 等），实现高可靠下载。
3. **前端业务智能联动**：
   - 在 `FileManage.vue` 中统一根据插件启用状态与策略调度播放：
     - 智能模式：标准 MP4 内嵌播放，ProRes / MOV 等广播格式自动调起 MPV 硬件加速；
     - 始终接管：所有视频均通过 MPV 播放；
     - 手动模式：普通格式内嵌，异常或手动点击时调起；
   - 本地磁盘挂载前自动检测 WinFsp 驱动状态，未就绪时给出明确指引。

---

## 4. 配置中心重构规划（后续版本实施）

### 4.1 痛点背景
- 当前配置中心承载较多重交互（多连接管理、多挂载点嵌套配置、认证证书、插件管理、传输与系统设置）；
- 520px 窄幅右侧抽屉导致卡片和表单极度拥挤局促，样式覆盖层级多。

### 4.2 重构目标（二选一待定）
- **方案 A：720px 宽幅现代化抽屉**：
  - 保留右侧抽屉交互习惯，宽度扩展为 720px；
  - 顶部导航升级为现代化 Segmented 胶囊滑块；
  - 内部卡片重构为清晰的 Key-Value 属性列表与独立卡片组。
- **方案 B：双栏居中设置面板（Settings Modal，如 VSCode / Postman）**：
  - 860px 宽幅居中模态窗口，左侧垂直分类导航，右侧舒展详情页；
  - 彻底解放交互区域，提供桌面资产管理工具顶级的视觉与操作体验。
- **实施原则**：100% 保持当前已有状态、Pinia store、持久化协议与业务数据流完全不变，仅重构布局与视觉层。
